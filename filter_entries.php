...
if(isset($_POST['dateStart']) && isset($_POST['dateEnd'])) {
    ...
    $query = "SELECT entre.*, eglise.designeglise FROM entre INNER JOIN eglise ON entre.ideglise = eglise.ideglise WHERE entre.dateentre BETWEEN :dateStart AND :dateEnd";
    ...
    if($stmt->rowCount() > 0) {
        ...
        while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo '<td class="text-center">' . $row['motifentre'] . '</td>';
            echo '<td class="text-center">' . $montantentre . '</td>';
            echo '<td class="text-center">' . $row['designeglise'] . '</td>';
            echo '<td class="text-center">' . $row['dateentre'] . '</td>';
            echo '<td class="text-center">';
            echo '<a href="#" title="Modifier" class="text-primary editBtn" id="'.$row['identre'].'" data-toggle="modal" data-target="#editModal"><i class="fas fa-edit fa-lg"></i></a>&nbsp;&nbsp;';
            echo '<a href="#" title="Supprimer" class="text-danger delBtn" id="'.$row['identre'].'"><i class="fas fa-trash-alt fa-lg"></i></a>';
            echo '</td>';
            echo '</tr>';
            $number++;
        }
